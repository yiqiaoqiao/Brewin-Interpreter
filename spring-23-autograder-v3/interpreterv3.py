...
class Interpreter(InterpreterBase):
    ...
    def is_valid_tclass_type(self, class_name, typename):
        return self.type_manager.is_valid_tclass_type(class_name, typename)
    ...
    def check_tclass_type_compatibility(
        self, class_name, typea, typeb, for_assignment=False
    ):
        return self.type_manager.check_tclass_type_compatibility(
            class_name, typea, typeb, for_assignment
        )
    ...
    def __map_class_names_to_class_defs(self, program):
        ...
        for item in program:
            if (
                ...
                _class = ClassDef(item, self)
                self.class_index[item[1]] = _class
                if item[0] == InterpreterBase.TEMPLATE_CLASS_DEF:
                    self.tclass_index[item[1]] = _class
    ...
    def __add_all_class_types_to_type_manager(self, parsed_program):
        ...
        for item in parsed_program:
            if item[0] == InterpreterBase.CLASS_DEF:
                ...
                class_name = item[1]
                parameterized_types = item[2]
                self.type_manager.add_tclass_type(class_name, parameterized_types)
