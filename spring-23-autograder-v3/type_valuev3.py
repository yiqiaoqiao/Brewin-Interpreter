...
class TypeManager:
    def __init__(self):
        ...
        self.map_tclass_parameterized_types = {}
    ...
    def add_tclass_type(self, class_name, parameterized_type):
        class_type = Type(class_name)
        self.map_typename_to_type[class_name] = class_type
        self.map_tclass_parameterized_types[class_name] = parameterized_type

    def is_valid_tclass_type(self, class_name, typename):
        if class_name in self.map_typename_to_type:
            return True
        if typename == "null":
            return True
        if not class_name in self.map_tclass_parameterized_types:
            # cannot find template class class_name
            return False
        parameterized_types = self.map_tclass_parameterized_types[class_name]
        return typename in parameterized_types

    def is_valid_type(self, typename):
        if "@" in typename:
            return typename.split("@")[0] in self.map_typename_to_type
    ...
    def check_tclass_type_compatibility(self, class_name, typea, typeb, for_assignment):
        typename_a = typea.type_name
        typename_b = typeb.type_name
        if "@" in typename_a:
            typename_a = typename_a.split("@")[0]
        if "@" in typename_b:
            typename_b = typename_b.split("@")[0]
        # print("typename_a: ", typename_a, " typename_b: ", typename_b)
        # if either type is invalid (E.g., the user referenced a class name that doesn't exist) then
        # return false
        if (not self.is_valid_tclass_type(class_name, typename_a)) or (
            not self.is_valid_tclass_type(class_name, typename_b)
        ):
            return False
        ...
        if typename_a in self.primitive_types or typename_b in self.primitive_types:
            return False
        ...
        if (
            typename_a == InterpreterBase.NULL_DEF
            or typename_b == InterpreterBase.NULL_DEF
        ):
            return True
        # all other cases
        return False
